...
const BACKEND_FLOWS_DIR = 'docs/backend/flows'; // Sequence diagrams live here in .md
const BACKEND_MAPS_DIR = 'docs/backend/interaction-maps'; // We should probably have a central backend map too
...
async function run() {
  ...
  for (const file of uiMaps) {
    ...
    graph.edges.forEach(edge => {
      if (!nodeIds.has(edge.from)) console.warn(`⚠️ Warning: Edge references missing node '${edge.from}'`);
      if (!nodeIds.has(edge.to)) console.warn(`⚠️ Warning: Edge references missing node '${edge.to}'`);
    });
