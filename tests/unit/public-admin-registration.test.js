...
describe("admin registration settings", () => {
	beforeEach(() => {
		document.body.innerHTML =
			'<div id="root" data-settings-tab="registration"></div>';
		localStorage.clear();
		vi.clearAllMocks();
	...
	it("renders the public registration toggle", async () => {
		...
		await vi.waitFor(() =>
			expect(container.querySelector("#public-reg-toggle")).not.toBeNull(),
		);
	});
	...
	it("saves public registration toggle changes immediately to the admin config API", async () => {
		...
		await vi.waitFor(() =>
			expect(mocks.apiFetch.mock.calls.length).toBeGreaterThan(
				initialCallCount,
			),
		);
		...
		expect(putCall[1].body).toBe(
			JSON.stringify({ public_registration: false }),
		);
	});
	...
	it("saves registration status changes immediately to the admin config API", async () => {
		...
		renderRegistrationSettings(container, data);
		await vi.waitFor(() =>
			expect(container.querySelector("#registration-status")).not.toBeNull(),
		);
		...
		expect(putCall[1].body).toBe(
			JSON.stringify({ public_registration_status: "active" }),
		);
	});
});
