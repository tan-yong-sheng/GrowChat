...
vi.mock("../../public/js/features/account/account.js", () => ({
	...
	openAccountSettingsDrawer: (...args) =>
		mocks.openAccountSettingsDrawer(...args),
	resolveAccountSectionFromPath: (...args) =>
		mocks.resolveAccountSectionFromPath(...args),
}));
...
vi.mock("../../public/js/shared/utils/model-sync.js", () => ({
	...
	consumeModelsInvalidation: (...args) =>
		mocks.consumeModelsInvalidation(...args),
}));
...
describe("public app bootstrap", () => {
	...
	it("delegates admin routes to the admin renderer", async () => {
		...
		mocks.fetchChats.mockResolvedValue({
			chats: [],
			limit: 30,
			offset: 0,
			has_more: false,
	...
	it("opens account routes through the account drawer", async () => {
		...
		expect(mocks.openAccountSettingsDrawer).toHaveBeenCalledWith({
			section: "connections",
		});
	});
	...
	it("removes stale route modals before rendering a new route", async () => {
		document.body.insertAdjacentHTML(
			"beforeend",
			'<div id="add-connection-modal"></div><div id="account-settings-drawer-modal"></div><div id="legacy-modal"></div>',
		);
		window.history.pushState({}, "", "/");
		mocks.getAuthState.mockReturnValue({ access_token: "token" });
		mocks.isAccessTokenUsable.mockReturnValue(true);
		...
		expect(
			document.querySelector('[data-account-settings-drawer-mount="1"]'),
		).toBeNull();
