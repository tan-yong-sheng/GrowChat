...
describe('public router', () => {
  ...
  it('returns health details without auth', async () => {
    db.first.mockResolvedValueOnce({ count: 0 });
    ...
    expect(body.publicRegistrationEnabled).toBe(true);
    ...
    db.first.mockResolvedValueOnce({ count: 2 });
    const initializedRes = await publicRouter(
      makeReq('/api/health'),
      { APP_NAME: 'GrowChat', DB: {}, SESSIONS: {}, MESSAGE_QUEUE: {} },
      {},
      null,
    ...
    const initializedBody = await initializedRes.json();
    expect(initializedBody.initialized).toBe(true);
    expect(initializedBody.publicRegistrationEnabled).toBe(true);
  });
