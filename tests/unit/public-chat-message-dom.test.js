...
describe('chat message dom helper', () => {
  it('updates message html and applies error state', () => {
    ...
    expect(renderAssistantMessageBody).toHaveBeenCalledWith(expect.objectContaining({
      messageId: 'm1',
      content: 'hello',
      isError: true,
      isStreaming: false,
    }));
    expect(container.querySelector('[data-message-content="m1"]').innerHTML).toBe('<p>rendered</p>');
