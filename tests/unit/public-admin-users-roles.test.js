...
describe("admin users roles", () => {
	...
	it("renders the roles list as the scroll container itself", async () => {
		...
		renderRolesPage(container);
		await vi.waitFor(() =>
			expect(container.querySelector("[data-role-list]")).not.toBeNull(),
		);
	...
	it("creates a new role immediately from the modal save", async () => {
		...
		document.querySelector('[data-permission-toggle="chat.read"]').checked =
			true;
		document
			.querySelector('[data-permission-toggle="chat.read"]')
			.dispatchEvent(new Event("change", { bubbles: true }));
		document.querySelector('[data-permission-toggle="chat.write"]').checked =
			true;
		document
			.querySelector('[data-permission-toggle="chat.write"]')
			.dispatchEvent(new Event("change", { bubbles: true }));
		...
		await vi.waitFor(() =>
			expect(mocks.createAdminRbacRole).toHaveBeenCalledWith(
		...
		await vi.waitFor(() =>
			expect(freshContainer.textContent).toContain("Support"),
		);
	});
	...
	it("renders a compact create-role modal with inline accessible controls", async () => {
		...
		const permissionPane = document.querySelector(
			"[data-role-permission-pane]",
		);
	...
	it("closes the modal without leaving stale role state behind", async () => {
		...
		await vi.waitFor(() =>
			expect(document.querySelector("[data-role-save]")).toBeNull(),
		);
	});
	...
	it("deletes roles immediately from the card list", async () => {
		...
		await vi.waitFor(() =>
			expect(mocks.deleteAdminRbacRole).toHaveBeenCalledWith("custom-1"),
		);
		await vi.waitFor(() =>
	...
	it("deletes roles immediately from the modal", async () => {
		...
		await vi.waitFor(() =>
			expect(mocks.deleteAdminRbacRole).toHaveBeenCalledWith("custom-2"),
		);
		await vi.waitFor(() =>
