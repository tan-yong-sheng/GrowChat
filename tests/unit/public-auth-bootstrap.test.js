...
describe("public auth bootstrap", () => {
	...
	it("defaults to register mode on fresh workspace", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(async (input) => {
		...
		expect(
			document.getElementById("name-wrap").classList.contains("hidden"),
		).toBe(false);
	});

	it("keeps login mode when workspace is already initialized", async () => {
		vi.stubGlobal(
			"fetch",
			vi.fn(async (input) => {
				const url = typeof input === "string" ? input : input.url;
	...
	it("shows a pending approval message instead of logging in when registration does not return tokens", async () => {
		...
		expect(
			document
				...
				.classList.contains("text-green-600"),
		).toBe(true);
	});
});
