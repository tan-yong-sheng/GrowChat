...
export function renderPoliciesSettings(container, data = {}) {
  ...
  const render = () => {
    ...
    container.innerHTML = `
      <div class="flex flex-col h-full min-h-0 animate-in fade-in duration-300">
        ...
        <div class="shrink-0 flex items-center justify-between pt-4 pb-3 px-0.5 border-t border-gray-100 bg-white sticky bottom-0 z-10">
          <div id="policy-dirty" class="text-xs text-amber-700 bg-amber-50 border border-amber-100 px-2.5 py-1 rounded-full ${dirty ? '' : 'invisible'}">Unsaved changes</div>
          <button id="policy-page-save" class="ml-auto px-5 py-1.5 text-sm font-medium transition rounded-full ${(!dirty || state.saving) ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-black text-white hover:bg-gray-900'}" ${(!dirty || state.saving) ? 'disabled' : ''}>
    ...
    container.querySelector('#policy-page-save')?.addEventListener('click', async () => {
      try {
        await savePolicies();
      } catch (err) {
        console.warn('Failed to save policies:', err);
      }
    });
