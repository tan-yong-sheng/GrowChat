    ...
    return Array.from(deduped.values())
      ...
      <div data-connection-row="${safeId}" class="py-2.5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between pr-2 border-b border-gray-50 last:border-0 ${conn.enabled === false ? 'opacity-70' : ''}">
        ...
        <div class="flex items-center justify-end gap-3 self-end sm:self-auto flex-wrap">
          ...
          <button data-id="${safeId}" class="edit-connection-btn p-1 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/20 transition-colors rounded ${conn.readOnly ? 'hidden' : ''}">
          ...
          <button data-id="${safeId}" class="connection-toggle relative inline-flex h-5 w-9 items-center shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${conn.enabled === false ? 'bg-gray-200' : 'bg-primary'}">
