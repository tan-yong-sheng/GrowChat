...
function renderConnectionToggle(conn, safeId) {
  const toggleBg = conn.enabled === false ? 'bg-gray-200' : 'bg-primary';
  const togglePosition = conn.enabled === false ? 'translate-x-0' : 'translate-x-4';
  return `<button data-id="${safeId}" class="connection-toggle relative inline-flex h-5 w-9 items-center shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${toggleBg}">
    <span class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${togglePosition}"></span>
  </button>`;
}

function renderConnectionAclButton(conn, safeId, canManageAcls) {
  const aclBtnHidden = conn.enabled === false || !canManageAcls ? 'hidden' : '';
  const aclBtnDisabled = canManageAcls ? '' : 'disabled aria-disabled="true"';
  return `<button data-id="${safeId}" class="connection-acl-btn inline-flex items-center justify-center h-8 w-8 rounded-lg text-gray-600 hover:bg-gray-100 transition ${aclBtnHidden}" title="Edit access rules" aria-label="Edit access rules" ${aclBtnDisabled}>
...
function renderConnectionEditButton(conn, safeId) {
  if (conn.readOnly) return '';
  return `<button data-id="${safeId}" class="edit-connection-btn p-1 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/20 transition-colors rounded ${conn.readOnly ? 'hidden' : ''}">
    ...
    function renderConnectionRow(conn) {
      ...
      <div data-connection-row="${safeId}" class="py-2.5 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between pr-2 border-b border-gray-50 last:border-0 ${conn.enabled === false ? 'opacity-70' : ''}">
        <div class="flex flex-col min-w-0">
          ...
          <div class="text-label-sm text-gray-700 font-mono">${safeUrl}</div>
          <div class="text-label-sm text-gray-700 mt-0.5">${safeProvider}</div>
          <div data-connection-disabled-badge class="mt-0.5 inline-flex w-fit items-center rounded-full border border-gray-200 bg-gray-100 px-1.5 py-0.5 text-label-xs font-semibold uppercase tracking-wide text-gray-700 ${conn.enabled === false ? '' : 'hidden'}">Disabled</div>
          ${conn.readOnly ? '<div class="text-label-sm text-gray-700 mt-0.5">Read-only connection</div>' : ''}
        </div>
        <div class="flex items-center justify-end gap-3 self-end sm:self-auto flex-wrap">
          ${renderConnectionAclButton(conn, safeId, canManageAcls)}
          ${renderConnectionEditButton(conn, safeId)}
          ${renderConnectionToggle(conn, safeId)}
        </div>
      </div>
    `;
    }
