...
export function renderEmailSettings(container, data) {
  const isActiveTab = () => container?.dataset?.settingsTab === 'email';
  const settingsState = data.emailSettings || (data.emailSettings = {
    currentValues: {
      resendApiKey: '',
    },
    initialValues: {
  ...
  data.settingsDirtyCheckers.email = () => false;
  data.settingsSaveHandlers.email = async () => false;
