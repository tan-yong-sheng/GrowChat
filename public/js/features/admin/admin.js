...
const loadAdminUsersGroupsHelpersModule = () =>
	import("./users/groups-helpers.js");
const loadAdminUsersGroupsListHelpersModule = () =>
	import("./users/groups-list-helpers.js");
...
const loadAdminSettingsConnectionsModule = () =>
	import("./settings/connections.js");
const loadAdminSettingsModelsModule = () => import("./settings/models.js");
const loadAdminSettingsIntegrationsModule = () =>
...
const overviewModules = {
		...
		usersModulesReadyPromise = Promise.all([
			...
			.then(
				([
					...
					usersModules.removeGroupById =
						groupsListHelpersModule.removeGroupById;
					usersModules.updateGroupMemberCount =
						groupsListHelpersModule.updateGroupMemberCount;
					usersModules.upsertGroup = groupsListHelpersModule.upsertGroup;
					usersModules.renderPoliciesSettings =
						policiesModule.renderPoliciesSettings;
	...
	const ensureSettingsModules = () => {
		if (settingsModules.renderConnectionsSettings)
			return Promise.resolve(settingsModules);
		...
		settingsModulesReadyPromise = Promise.all([
			...
			.then(([connectionsModule, modelsModule, integrationsModule]) => {
				settingsModules.renderConnectionsSettings =
					connectionsModule.renderConnectionsSettings;
				settingsModules.renderModelsSettings =
					modelsModule.renderModelsSettings;
				settingsModules.renderIntegrationsSettings =
					integrationsModule.renderIntegrationsSettings;
	...
	const ensureSystemModules = () => {
		if (systemModules.renderRegistrationSettings)
			return Promise.resolve(systemModules);
		...
		systemModulesReadyPromise = Promise.all([
			...
			.then(([registrationModule, emailDeliveryModule, securityOverviewModule, auditLogsModule]) => {
				systemModules.renderRegistrationSettings =
					registrationModule.renderRegistrationSettings;
				systemModules.renderEmailDeliverySettings =
				emailDeliveryModule.renderEmailDeliverySettings;
				systemModules.renderSecuritySettings =
				securityOverviewModule.renderSecurityOverview;
				systemModules.renderAuditLogs = ({ apiFetch, showToast }) => auditLogsModule.renderAuditLogsSection({ apiFetch, showToast });
	...
	const sortUsers = (users) =>
		users.slice().sort((a, b) => {
			...
			const roleDiff =
				(roleOrder[a.primary_role] ?? 2) - (roleOrder[b.primary_role] ?? 2);
			if (roleDiff !== 0) return roleDiff;
			const statusDiff =
				(statusOrder[a.account_status] ?? 2) -
				(statusOrder[b.account_status] ?? 2);
	...
	const prependCachedUser = (user) => {
		clearUsersCache();
		data.users = sortUsers([user, ...data.users]).slice(
			0,
	...
	const renderSubContent = async () => {
		...
		const subContentEl =
			container.querySelector("#admin-sub-body") ||
			container.querySelector("#admin-sub-content");
		...
		if (needsModuleLoad) {
			...
			try {
				...
				subContentEl.innerHTML = renderErrorState(
					err?.message || "Failed to load admin section.",
				);
				renderMainActionFooter();
				updateMainActionFooter();
				return;
		...
		if (data.error) {
			...
			usersModules.renderGroupsOverview?.(subContentEl, data, {
				...
				onCreate(group) {
					data.groups =
						usersModules.upsertGroup?.(data.groups, group) || data.groups;
					renderSubContent();
				},
				onUpdate(group) {
					data.groups =
				...
				onDelete(groupId) {
					data.groups =
						usersModules.removeGroupById?.(data.groups, groupId) || data.groups;
					renderSubContent();
				},
	...
	function mountShell() {
		const previousCleanup =
			typeof container.__cleanup === "function" ? container.__cleanup : null;
		...
		container.innerHTML = renderWorkspaceShell({
          ...
          ${renderWorkspaceTopNav({
						...
						leadingSlotHtml: renderWorkspaceTopNavSidebarToggle({
							...
							title: "Open Sidebar",
							className:
								"p-2 mr-2 hover:bg-gray-100 rounded-lg transition text-gray-700 md:hidden",
						}),
					})}
