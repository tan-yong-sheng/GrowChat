...
export function renderRoleRow(role) {
  const initials =
    String(role.name || '?')
      ...
      .toUpperCase() || '?';
  const rowClasses =
    'flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-3.5 py-3 rounded-lg hover:bg-gray-50/80 transition-all group cursor-pointer border border-transparent hover:border-gray-100/50';
  const deleteButton = role.system
    ? ''
    : `
        <button type="button" class="p-2 hover:bg-red-50 rounded-md text-gray-400 hover:text-red-500 transition-all btn-delete-role" data-role-delete="${escapeHtml(role.id)}" aria-label="Delete role">
  ...
  return `
    ...
    >
      <div class="flex items-center gap-3.5 min-w-0">
        <div class="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 text-label-sm font-semibold shrink-0">
          ${escapeHtml(initials)}
        </div>
        <div class="flex flex-col min-w-0">
          ...
          <div class="text-label-sm text-gray-500 font-medium">${escapeHtml(role.description)} · ${escapeHtml(formatRoleSummary(role))}</div>
        </div>
      </div>
      <div class="flex items-center justify-end gap-1.5 shrink-0 self-end sm:self-auto">
        <button type="button" class="p-2 hover:bg-gray-200 rounded-md text-gray-400 transition-all btn-edit-role" data-role-edit="${escapeHtml(role.id)}" aria-label="Edit role">
