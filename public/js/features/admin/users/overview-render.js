...
export function renderUserRows(users) {
  return users
    ...
    <tr data-user-row="${u.id}" class="bg-white text-xs hover:bg-gray-50/50 transition-colors">
      ...
      <td class="px-3 py-4 text-gray-400 font-normal uppercase text-label-sm whitespace-nowrap">${u.last_active_at ? timeSince(u.last_active_at * 1000) : 'N/A'}</td>
      <td class="px-3 py-4 text-gray-400 font-normal text-label-sm whitespace-nowrap">${u.created_at ? new Date(u.created_at * 1000).toLocaleDateString() : 'N/A'}</td>
    ...
    body: `
      <div class="space-y-4">
        <div class="flex items-center gap-5 border-b border-gray-100 -mx-5 px-5 -mt-1">
          <button type="button" data-add-user-tab="form" class="pb-3 text-base font-medium text-gray-900 border-b-2 border-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/20 rounded transition">Form</button>
          <button type="button" data-add-user-tab="csv" class="pb-3 text-base font-medium text-gray-400 border-b-2 border-transparent focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary/20 rounded transition">CSV Import</button>
        </div>
