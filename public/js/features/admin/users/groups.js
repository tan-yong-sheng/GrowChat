...
import { sortGroups, escapeHtml } from './groups-list-helpers.js';
...
function renderGroupsHeader(count) {
  return `
...
function renderGroupsSearchBar() {
  return `
...
function renderGroupsBody({ isLoading, error, groups }) {
  if (isLoading) {
    return `<div class="p-10 text-center text-sm text-gray-700">Loading groups...</div>`;
  }
  if (error) {
    return `<div class="p-10 text-center text-sm text-red-500">${error}</div>`;
  }
  if (groups.length) {
    return renderGroupsList(groups);
  }
  return renderEmptyState();
}

function renderGroupsPanel({ isLoading, error, groups }) {
  return `
    <div class="flex-1 min-h-0 py-2.5 bg-white rounded-lg border border-gray-100/50 shadow-sm flex flex-col">
      ${renderGroupsSearchBar()}
      ${renderGroupsBody({ isLoading, error, groups })}
    </div>
  `;
}

function attachGroupsCreateHandler(container, actions, data) {
  container.querySelector('#create-group-btn')?.addEventListener('click', async () => {
    await actions.onCreate?.({ navigationState: data });
  });
}

function attachGroupsEditHandlers(container, actions, data, deleteGroup) {
  container.querySelectorAll('.btn-edit-group').forEach((btn) =>
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      await actions.onUpdate?.(btn.dataset.groupId, {
...
function attachGroupsDeleteHandlers(container, deleteGroup) {
...
function attachGroupsSearchHandlers(container) {
...
export function renderGroupsOverview(container, data, actions = {}) {
  ...
  container.innerHTML = `
    <div class="flex flex-col flex-1 min-h-0 animate-in fade-in duration-300">
      ${renderGroupsHeader(groups.length)}
      ${renderGroupsPanel({ isLoading, error, groups })}
    </div>
  `;

  attachGroupsCreateHandler(container, actions, data);
  attachGroupsEditHandlers(container, actions, data, deleteGroup);
  attachGroupsDeleteHandlers(container, deleteGroup);
  attachGroupsSearchHandlers(container);
}
