...
function isUsingFilter(query, provider) {
 ...
 */
function renderProviderOptionsHtml(providerOptions, models, currentProvider) {
...
function buildNextPreferencesPayload(sectionState, currentSettings) {
  return {
...
function mergeSavedAttachmentCaps(savedSettings, mergedCaps, sectionState) {
...
function buildCombinedModelsArray(visibleModels, hiddenModels, mergedCaps) {
  return [
...
function applyRollbackState(sectionState, rollback) {
...
function parseDisabledModelIds(payload) {
  return new Set(
