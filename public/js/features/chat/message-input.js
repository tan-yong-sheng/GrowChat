...
export function renderMessageInput(container, onSend) {
  container.innerHTML = `
    ...
    <form id="composer" class="relative bg-surface-container rounded-full p-1.5 flex items-end transition focus-within:bg-surface focus-within:ring-1 border border-transparent focus-within:border-gray-200">
       <div class="relative flex-shrink-0 ml-1 flex items-center gap-1">
         ...
         <div id="attach-menu" class="hidden absolute bottom-full left-0 mb-2 w-52 rounded-2xl border border-gray-100 bg-surface shadow-xl p-1 z-30">
           <button type="button" id="attach-upload" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-neutral-bg rounded-xl flex items-center gap-2.5 whitespace-nowrap">
             <i class="bi bi-paperclip text-[15px] leading-none"></i>
             <span class="whitespace-nowrap">Upload Files</span>
           </button>
           <button type="button" id="attach-capture" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-neutral-bg rounded-xl flex items-center gap-2.5">
         ...
         <div id="tools-menu" class="hidden absolute bottom-full left-12 mb-2 w-60 rounded-2xl border border-gray-100 bg-surface shadow-xl p-1 z-30">
       ...
       <div class="flex-shrink-0 flex items-center mb-1 mr-1 gap-1 relative">
         <div id="loading-spinner" class="hidden absolute inset-0 bg-surface-container items-center justify-center rounded-full transition-all z-10" aria-live="polite">
        ...
        <button type="button" id="mic-btn" class="p-2 text-gray-500 hover:text-black hover:bg-gray-200 rounded-full transition" title="Voice input" aria-label="Voice input">
         ...
         <button id="send-btn" class="hidden p-2 bg-primary text-white rounded-full hover:bg-primary-hover active:scale-95 transition disabled:opacity-50" title="Send message (Ctrl+Enter / Cmd+Enter)" aria-label="Send message">
