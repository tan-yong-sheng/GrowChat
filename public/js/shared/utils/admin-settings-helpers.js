 ...
 * @param {string} statusId - Status text element ID
 * @param {boolean} isOn - Current state
 * @param {Function} [updateVisibility] - Optional callback to update visibility
 */
export function updateSettingsToggle(container, toggleId, statusId, isOn, updateVisibility) {
  ...
  if (!toggle) return;
  const state = getSettingsToggleState(isOn);
  toggle.setAttribute('aria-pressed', state.ariaPressed);
  toggle.classList.toggle('bg-black', state.isOn);
  toggle.classList.toggle('bg-gray-200', !state.isOn);
  const knob = toggle.querySelector('span');
  if (knob) {
    knob.classList.toggle('translate-x-4', state.isOn);
    knob.classList.toggle('translate-x-0', !state.isOn);
  }
 ...
 * @param {Function} ctx.onOptimisticUpdate - Called with newValue before request
...
export async function updateAdminConfig({
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(err?.error || err?.message || `Failed to update ${errorPrefix}`);
    }
