...
export function renderModelsPaginationHtml({
  ...
  return `
    <div class="shrink-0 sticky bottom-0 z-20 border-t border-gray-100 bg-white shadow-[0_-1px_0_rgba(17,24,39,0.04)]" data-models-pagination>
  ...
  if (providerSelect) {
    ...
    if (providerOptionsMarkup) {
      const fragment = document
        .createRange()
        .createContextualFragment(providerOptionsMarkup.trim());
...
export function syncModelsTableState(
  container,
  { loading, rowsHtml, emptyMessage, tbodyId = 'models-table-body', emptyColSpan = 4 } = {}
) {
  if (!container) return;
