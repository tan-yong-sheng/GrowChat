...
export function renderFilesModal(container) {
  container.innerHTML = buildViewportModalShellMarkup({
    ...
    zIndex: 100,
    title: '',
    overlayClass: 'absolute inset-0 bg-primary/40 backdrop-blur-sm transition-opacity',
    shellClass:
      'relative z-10 w-full max-w-2xl bg-white rounded-lg shadow-2xl overflow-hidden border border-gray-100 flex flex-col max-h-[90vh] md:h-[600px]',
    header: `
      <div class="p-6 border-b border-gray-100 flex items-center justify-between gap-4">
        ...
        <div class="hidden md:block flex-grow max-w-xs">
          <input id="files-search-input" type="text" placeholder="Search files..." class="w-full px-3 py-2 text-sm border border-gray-200 rounded-md placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2 focus:ring-offset-white" />
        </div>
        <div class="flex items-center gap-3">
          <label for="file-upload-input" class="inline-flex min-h-11 items-center gap-2 bg-primary text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-primary-hover transition cursor-pointer">
             <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
             Upload
          </label>
          <input type="file" id="file-upload-input" class="hidden" multiple />
          <button type="button" id="close-files-modal" class="inline-flex h-11 w-11 items-center justify-center rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300 focus-visible:ring-offset-2 focus-visible:ring-offset-white" aria-label="Close files">
    ...
    footer: `
      <div class="p-6 border-t border-gray-100 flex items-center justify-between bg-gray-50/50">
        ...
        <button id="attach-selected-btn" type="button" class="inline-flex min-h-11 items-center justify-center bg-gray-100 text-gray-400 border border-gray-200 px-6 py-2 rounded-md text-sm font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-300 focus-visible:ring-offset-2 focus-visible:ring-offset-white disabled:opacity-100 disabled:cursor-not-allowed" disabled>
           Attach to Chat
        </button>
      </div>
    `,
