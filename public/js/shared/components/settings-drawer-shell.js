  ...
  return `
    <div id="${escapeHtml(rootId)}" class="fixed inset-0 z-[220]" role="dialog" aria-modal="true" aria-labelledby="${escapeHtml(`${rootId}-title`)}">
      ...
      <div class="absolute inset-0 flex items-center justify-center p-3 sm:p-4 lg:p-5">
        <div class="flex h-[calc(100dvh-1.5rem)] w-[calc(100vw-1.5rem)] max-w-none flex-col overflow-hidden rounded-[1.2rem] border border-gray-200 bg-[#fafafa] text-gray-900 shadow-[0_24px_90px_rgba(15,23,42,0.18)] sm:h-[calc(100vh-2rem)] sm:w-[calc(100vw-2rem)] sm:rounded-[1.4rem] lg:h-[calc(100vh-2.5rem)] lg:w-[calc(100vw-2.5rem)]">
