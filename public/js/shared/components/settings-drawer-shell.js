...
export function renderSettingsDrawerShell({
  ...
  return `
    <div id="${escapeHtml(rootId)}" class="fixed inset-0 z-[220]" role="dialog" aria-modal="true" aria-labelledby="${escapeHtml(`${rootId}-title`)}">
      <div id="${escapeHtml(overlayId)}" class="absolute inset-0 bg-primary/25 backdrop-blur-sm" aria-hidden="true"></div>
      <div class="absolute inset-0 flex items-center justify-center p-3 sm:p-4 lg:p-5">
        <div class="flex h-[calc(100dvh-1.5rem)] w-[calc(100vw-1.5rem)] max-w-none flex-col overflow-hidden rounded-lg border border-gray-200 bg-[#fafafa] text-gray-900 sm:h-[calc(100vh-2rem)] sm:w-[calc(100vw-2rem)] sm:rounded-lg lg:h-[calc(100vh-2.5rem)] lg:w-[calc(100vw-2.5rem)]">
          <div class="shrink-0 border-b border-gray-100 bg-white/95 backdrop-blur-md px-4 py-3 sm:px-5 sm:py-4">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <div class="flex items-center gap-2">
                  <span class="inline-flex items-center rounded-full border border-emerald-100 bg-emerald-50 px-2 py-0.5 text-label-sm font-semibold uppercase tracking-wide text-emerald-700">
