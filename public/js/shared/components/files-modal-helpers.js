...
export function renderFilesListMarkup(files = [], currentState = {}) {
  return (Array.isArray(files) ? files : [])
    .map((file) => {
      ...
      return `
      <div class="group flex items-center gap-4 p-3 rounded-lg border ${isSelected ? 'border-black bg-gray-50' : 'border-gray-100 hover:bg-gray-50'} transition-all cursor-pointer" data-file-id="${file.id}">
        <div class="flex-shrink-0 w-10 h-10 rounded-md bg-gray-100 flex items-center justify-center text-gray-400 group-hover:bg-white transition-colors">
        ...
        <div class="flex-shrink-0 flex items-center gap-1">
          ...
          <div class="w-6 h-6 rounded-full border-2 ${isSelected ? 'bg-primary border-black' : 'border-gray-200'} flex items-center justify-center transition-colors">
