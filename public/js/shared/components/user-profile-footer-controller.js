...
async function renderUserResourcesRoute() {
  const { renderUserResources } = await import('../../features/user/resources.js');
  return renderUserResources(document.getElementById('app'));
}
...
export async function createUserProfileFooter() {
  ...
  function ensureResourcesMenu() {
    if (!menu) return;
    if (menu.querySelector('[data-action="resources"]')) return;
    const container = menu.querySelector('[class~="space-y-0.5"]');
    if (!container) return;
    const preferencesBtn = container.querySelector('[data-action="preferences"]');
    const resourcesBtn = document.createElement('button');
    resourcesBtn.setAttribute('data-action', 'resources');
    resourcesBtn.className = 'menu-item flex items-center gap-3 w-full text-left px-3 py-2 rounded-xl text-sm hover:bg-gray-50 transition-colors text-gray-700 group';
    resourcesBtn.innerHTML = `
      ...
      <span>Resources</span>
    `;
    if (preferencesBtn && preferencesBtn.nextSibling) {
      container.insertBefore(resourcesBtn, preferencesBtn.nextSibling);
    } else if (preferencesBtn) {
      container.insertBefore(resourcesBtn, preferencesBtn.nextSibling);
    } else {
      container.insertBefore(resourcesBtn, container.firstChild);
    }
  }
  ...
  element.addEventListener('click', async (e) => {
    ...
    if (action === 'admin') {
      ...
      await renderUserResourcesRoute();
