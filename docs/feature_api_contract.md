...
## P2: Prompt Templates API

### 1. List Prompts
**Endpoint**: `GET /api/prompts/list`
**Auth**: Required
**Description**: List user and global prompts
...
**Request**:
```bash
curl -X GET "http://localhost:8787/api/prompts/list?category=writing&limit=50" \
  -H "Authorization: Bearer {access_token}"
```
...
**Request**:
```bash
curl -X POST http://localhost:8787/api/prompts/create \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Summarize",
...
**Endpoint**: `GET /api/prompts/command/:command`
**Auth**: Required
**Description**: Fast lookup prompt by command identifier

**Request**:
```bash
curl -X GET http://localhost:8787/api/prompts/command/translate \
  -H "Authorization: Bearer {access_token}"
```
...
**Auth**: Required
**Description**: Retrieve specific prompt

**Request**:
```bash
curl -X GET http://localhost:8787/api/prompts/{prompt_id} \
  -H "Authorization: Bearer {access_token}"
```
...
**Auth**: Required
**Description**: Update prompt content/title/category
...
curl -X PUT http://localhost:8787/api/prompts/{prompt_id} \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{"content": "Updated prompt text..."}'
```

**Response** (200):
```json
{
  "prompt": { /* updated object */ }
}
```

### 6. Toggle Prompt Active State
**Endpoint**: `POST /api/prompts/:id/toggle`
**Auth**: Required
**Description**: Soft-delete or reactivate prompt

**Request**:
```bash
curl -X POST http://localhost:8787/api/prompts/{prompt_id}/toggle \
  -H "Authorization: Bearer {access_token}"
```
...
**Auth**: Required
**Description**: Soft-delete prompt (sets is_active=0)

**Request**:
```bash
curl -X DELETE http://localhost:8787/api/prompts/{prompt_id} \
  -H "Authorization: Bearer {access_token}"
```
...
- **006_prompts.sql** - Creates `prompts` table with indexes
...
- [ ] Add prompt templates dropdown to message input
