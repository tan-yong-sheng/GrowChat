...
As of the repo-reduction pass (see `docs/REDUCTION_HANDOFF_PLAN.md` row 4), the async document text extraction pipeline has been removed. No background jobs are scheduled for uploads.
...
- The `/api/files/:id/content` endpoint returns `null` with a note that extraction is unavailable.
