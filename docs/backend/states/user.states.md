...
- `null` → `active` (Google OAuth auto-provisioning — email verified by Google, account immediately active with `member` role).
...
- Google OAuth users with `pending` status will be redirected to `/auth.html?oauth_error=pending_account` on callback, preventing token issuance.
- Google-only users (with `password_hash: 'oauth:no-password'`) cannot authenticate via email/password — they must use Google OAuth.
