...
export async function adminRouter(req, env, ctx, user, path) {
	...
	if (req.method === "GET" && path === "/api/admin/openai/connections/access") {
		try {
         ...
         ORDER BY is_system DESC, name ASC`,
			...
			const rules = await loadConnectionAclRules(
				db,
				null,
				ids.length ? ids : null,
			);
		...
		try {
			...
			for (const update of updates) {
				const connectionId = String(
					update?.connection_id || update?.connectionId || "",
				).trim();
				if (!connectionId) {
					return error(req, "connection_id is required", 400);
				}
				const currentConnection = (
					Array.isArray(allConnections) ? allConnections : []
				).find((conn) => String(conn.id || "") === String(connectionId));
				...
				if (invalidPrincipalTypes.length) {
					return error(
						req,
						"Invalid principal_type for connection access",
						400,
						{
				...
				const { statements: aclStatements } =
					buildConnectionAclRuleSaveStatements(
						db,
						connectionId,
						filteredRules,
	...
	const connectionAccessMatch = path.match(
	...
	if (req.method === "GET" && path === "/api/admin/tool-servers/access") {
		try {
         ...
         ORDER BY is_system DESC, name ASC`,
			...
			const rules = await loadToolServerAclRules(
				db,
				null,
				ids.length ? ids : null,
			);
	...
	if (req.method === "PUT" && path === "/api/admin/tool-servers/access") {
		...
		try {
			...
			for (const update of updates) {
				const toolServerId = String(
					update?.tool_server_id || update?.toolServerId || "",
				).trim();
				if (!toolServerId) {
				...
				if (invalidPrincipalTypes.length) {
					return error(
						req,
						"Invalid principal_type for MCP server access",
						400,
						{
							invalid: Array.from(new Set(invalidPrincipalTypes)),
						},
	...
	const toolServerAccessMatch = path.match(
	...
	if (req.method === "GET" && path === "/api/admin/config") {
		try {
			const publicRegistration = await getConfigBool(
				db,
				"public_registration",
				true,
			);
			...
			const defaultModelIdRaw = await getConfigValue(
				db,
				"default_model_id",
				null,
			);
			const registrationStatus =
				String(registrationStatusRaw || "pending")
					...
					? "active"
					: "pending";
			const defaultModelId = defaultModelIdRaw
				? String(defaultModelIdRaw).trim()
				: null;
			return json(req, {
	...
	if (req.method === "PUT" && path === "/api/admin/config") {
		...
		if (
			...
			typeof body.public_registration !== "boolean"
		) {
		...
		if (hasRegistrationStatus) {
			...
			normalizedRegistrationStatus = String(body.public_registration_status)
				.trim()
				.toLowerCase();
			if (!["active", "pending"].includes(normalizedRegistrationStatus)) {
				return error(
					req,
					'public_registration_status must be "active" or "pending"',
					400,
				);
			}
		}
		...
		if (hasDefaultModel) {
			if (body.default_model_id === null || body.default_model_id === "") {
				...
				if (
					normalizedDefaultModel.length > 200 ||
					/\s/.test(normalizedDefaultModel)
				) {
		...
		try {
			...
			return json(req, {
				public_registration: hasPublicRegistration
					? body.public_registration
					: undefined,
				public_registration_status: hasRegistrationStatus
					? normalizedRegistrationStatus
					: undefined,
				default_model_id: hasDefaultModel
	...
	if (req.method === "PUT" && path === "/api/admin/model-attachment-caps") {
		...
		const replaceCaps =
			body.caps && typeof body.caps === "object" && !Array.isArray(body.caps);
		const updates = Array.isArray(body.updates) ? body.updates : [];
		...
		try {
			...
			for (const update of updates) {
				...
				const current =
					caps[modelId] && typeof caps[modelId] === "object"
						? caps[modelId]
						: {};
			...
			try {
				...
				if (Array.isArray(parsed)) {
					manualConnections = parsed.map((conn, index) => ({
						...
						providerType: String(
							conn?.providerType || "openai-compatible",
						).toLowerCase(),
						providerFamily:
							normalizeProviderFamily(
								conn?.providerType || conn?.providerFamily,
							) || "openai",
						hasKey: Boolean(
							conn?.key || conn?.keyMasked || conn?.hasKey || conn?.has_key,
						),
						keyMasked:
							conn?.keyMasked ||
							(conn?.key ? `••••${String(conn.key).slice(-4)}` : ""),
						key: undefined,
						readOnly: false,
						source: "config",
	...
	if (req.method === "POST" && path === "/api/admin/openai/connections/test") {
		...
		const providerFamily =
			normalizeProviderFamily(body.providerType || body.providerFamily) ||
			"openai";
		const url = String(body.url || "").trim();
		const connectionId = String(body.id || body.connectionId || "").trim();
		const requiresUrl = isConnectionUrlRequired(providerType);
		const baseUrl =
			url || getConnectionDefaultBaseUrl(providerType || providerFamily);
		if (requiresUrl && !url) {
			return error(
				req,
		...
		try {
			...
			if (body.id) {
				...
				const index = servers.findIndex(
					(entry) => String(entry.id) === String(body.id),
				);
				if (index !== -1) {
			...
			if (body?.id) {
				try {
					...
					console.warn(
						"Failed to persist tool server error:",
	...
	if (req.method === "POST" && path === "/api/admin/tool-servers/oauth/start") {
		...
		const oauthClientName = String(
			body.oauth_client_name ||
				server.oauth_client_name ||
				"GrowChat MCP Client",
		).trim();
		const oauthScope = String(
			body.oauth_scope || server.oauth_scope || "",
		).trim();
		const authServerUrl = String(
			body.oauth_authorization_server ||
				server.oauth_authorization_server ||
				serverUrl,
		).trim();

		const redirectUri =
			new URL(req.url).origin + "/api/admin/tool-servers/oauth/callback";
		...
		let clientId = String(
			body.oauth_client_id || server.oauth_client_id || "",
		).trim();
		let clientSecret = String(
			body.oauth_client_secret || server.oauth_client_secret || "",
		).trim();
		let registrationEndpoint =
			metadata?.registration_endpoint ||
			server.oauth_registration_endpoint ||
			"";
		...
		const authorizationEndpoint =
			...
			new URL("/authorize", authServerUrl).toString();
		const tokenEndpoint =
			metadata?.token_endpoint || new URL("/token", authServerUrl).toString();
	...
	if (
		...
		const tokenEndpoint =
			...
			new URL(
				"/token",
				server.oauth_authorization_server || server.url,
			).toString();
		...
		const tokenAuthMethod =
			normalizeTokenAuthMethod(server.oauth_token_auth_method) ||
			"client_secret_post";
	...
	if (req.method === "PUT" && path === "/api/admin/tool-servers") {
		...
		const existingById = new Map(
			existing.map((entry) => [String(entry.id), entry]),
		);
		const sanitized = servers
			.map((server) => {
				const merged = mergeToolServer(
					existingById.get(String(server.id)),
					server,
				);
				return merged;
			})
	...
	if (req.method === "PUT" && path === "/api/admin/openai/connections") {
		...
		const modelUpdatesInput = Array.isArray(body.model_updates)
			? body.model_updates
			: [];
		...
		try {
			...
			currentConnectionMap = new Map(
				(Array.isArray(currentConnections) ? currentConnections : []).map(
					(connection) => [String(connection.id || ""), connection],
				),
			);
			...
			sanitized = connections
				...
				.map((conn) => {
					const existingConnection = currentConnectionMap.get(
						String(conn.id || ""),
					);
					...
					const providerFamily =
						normalizeProviderFamily(providerType || conn.providerFamily) ||
						"openai";
					const rawUrl = String(conn.url || "").trim();
					const requiresUrl = isConnectionUrlRequired(providerType);
					const url =
						rawUrl ||
						getConnectionDefaultBaseUrl(providerType || providerFamily);
		...
		try {
			...
			for (const entry of accessUpdatesInput) {
				const connectionId = String(
					entry?.connection_id || entry?.connectionId || "",
				).trim();
			...
			for (const entry of normalizedAccessUpdates) {
				statements.push(
					db.prepare(
						"DELETE FROM connection_acl_rules WHERE connection_id = ?",
						[entry.connection_id],
					),
				);
	...
	if (req.method === "GET" && path === "/api/admin/email-config") {
		try {
			const resendApiKeyConfigured = await getConfigValue(
				db,
				"resend_api_key",
				null,
			);
