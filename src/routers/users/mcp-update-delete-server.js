...
export async function handleUpdateMcpServer(req, env, userSub, serverId) {
  ...
  try {
    ...
    if (!updated) return error(req, 'MCP server not found', HTTP_STATUS.NOT_FOUND);
    ...
    return error(req, err?.message || 'Failed to update MCP server', HTTP_STATUS.BAD_REQUEST);
  }
}
...
export async function handleDeleteMcpServer(req, env, userSub, serverId) {
  ...
  try {
    ...
    if (!deleted) return error(req, 'MCP server not found', HTTP_STATUS.NOT_FOUND);
    await logAuditEvent(env, {
      actor_id: userSub,
    ...
    return error(req, err?.message || 'Failed to delete MCP server', HTTP_STATUS.BAD_REQUEST);
  }
}
