...
describe('chat stream utils', () => {
  it('builds persisted assistant content with reasoning suffix', () => {
    expect(buildPersistedAssistantContent('hello', ' think ')).toBe(
  ...
  it('decides when assistant content should be persisted', () => {
    expect(shouldPersistAssistantContent({ now: 2000, lastPersistAt: 0, fullText: 'abc' })).toBe(
      true
    );
    expect(shouldPersistAssistantContent({ now: 1000, lastPersistAt: 900, fullText: 'abc' })).toBe(
      false
    );
