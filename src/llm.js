...
export async function streamLLM(env, model, messages, options = {}) {
	...
	if (!parsed) {
		...
		if (enabledConnections.length > 1) {
			throw new Error(
				"Model id must include provider prefix when multiple providers are enabled",
			);
		}
		...
		primaryConn = allConnections.find((conn) => {
			...
			const family =
				normalizeProviderFamily(conn.providerFamily || conn.providerType) ||
				"openai";
	...
	const providerFamily =
		normalizeProviderFamily(
			primaryConn.providerFamily || primaryConn.providerType,
		) || "openai";
