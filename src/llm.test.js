...
vi.mock("./llm/connections.js", async (importOriginal) => {
	...
	return {
		...actual,
		getAllOpenAIConnectionConfigs: (...args) =>
			mocks.getAllOpenAIConnectionConfigs(...args),
	};
});
...
describe("llm.js - LLM Streaming", () => {
	...
	describe("streamLLM", () => {
		it("should throw when model is not provided", async () => {
			await expect(streamLLM(mockEnv, "", [])).rejects.toThrow(
				"Model is required",
			);
			await expect(streamLLM(mockEnv, null, [])).rejects.toThrow(
				"Model is required",
			);
			await expect(streamLLM(mockEnv, undefined, [])).rejects.toThrow(
				"Model is required",
			);
		});
		...
		it("should reject Workers AI models", async () => {
			...
			await expect(
				streamLLM(mockEnv, "@cf/meta/llama-2-7b", messages),
			).rejects.toThrow("Workers AI models are disabled");
		});
		...
		it("should use default connect timeout when env timeout is missing", async () => {
			vi.useFakeTimers();
			...
			try {
				const promise = streamLLM(mockEnv, "gpt-4", []);
		...
		it("should honor env connect timeout override", async () => {
			vi.useFakeTimers();
			global.fetch = vi.fn().mockImplementation(
				(_, init = {}) =>
					new Promise((_, reject) => {
			...
			try {
				const promise = streamLLM(
					{ ...mockEnv, LLM_CONNECT_TIMEOUT_MS: "50" },
					"gpt-4",
					[],
				);
				const rejection = promise.catch((err) => err);
	...
	describe("SseLineParser", () => {
		...
		describe("push", () => {
			...
			it("should emit Gemini tool call deltas", () => {
				...
				expect(
					events.find((event) => event.type === "finish_reason")?.reason,
				).toBe("tool_calls");
			});
			...
			it("should emit Anthropic tool call deltas", () => {
				...
				const result = chunks
					.map((chunk) => parserWithEvents.push(chunk))
					.join("");

				expect(result).toBe("");
				expect(events.some((event) => event.type === "tool_call_delta")).toBe(
					true,
				);
				expect(
					events.find((event) => event.type === "finish_reason")?.reason,
